...
(setq org-todo-keywords
      '((sequence "TODO(t)" "NEXT(n)" "WAIT(w)" "HOLD(h)" "|" "DONE(d)" "CNCL(c)")
        (sequence "PROJ(p)" "LOOP(l)" "IDEA(i)" "|" "DONE(d)" "CNCL(c)")
        (sequence "[ ](T)" "[-](S)" "[?](W)" "|" "[X](D)")
        (sequence "|" "OKAY(o)" "YES(y)" "NO(n)")))

      ;; '((sequence "TODO(t)" "NEXT(n)" "HOLD(h)" "LOOP(l)" "PROJ(p)" "|" "DONE(d)" "|" "CANCEL(c)")))
#+end_src
...
(setq org-ellipsis " ▾ "
      ...
      '((?A . 'nerd-icons-red)
        ...
        (?E . 'nerd-icons-blue)))
;;(custom-set-faces!
;;  '(+org-todo-active :inherit (bold org-todo) :foreground "#7bc275"))
;;
#+end_src
...
(use-package! org-caldav
  :config
  (setq org-caldav-url "https://posteo.de:8443/calendars/zeo")
  ;;       org-caldav-calendar-id "default"
  ;;       org-caldav-inbox "~/org/gtd/calendar.org"))

 (setq org-caldav-calendars
  '((:calendar-id "default"
     ;; :files ("~/org/work.org")
     :inbox "~/org/gtd/cal-work.org")
    (:calendar-id "pjygij"
     :files ("~/org/gtd/org-gtd-tasks.org")
     :inbox "~/org/gtd/cal-pers.org"))))
     ;; :skip-conditions (regexp "soccer")
#+end_src
** calfw

#+call: confpkg("!Pkg calfw")

# #+begin_src emacs-lisp
# (use-package! calfw
#   :config
#   ;; (require 'calfw-ical)
#   (setq cfw:open-ical-calendar "https://posteo.de/calendars/ics/uom0kta3bomsfffh0hhxs7jmrkqz9mbm")
#   )
# #+end_src
...
(defun calendar-helper () ;; doesn't have to be interactive
  (cfw:open-calendar-buffer
   ...
   (list
    ...
    (cfw:ical-create-source "Posteo-Work" "https://posteo.de/calendars/ics/uom0kta3bomsfffh0hhxs7jmrkqz9mbm" "Green")
    (cfw:ical-create-source "Posteo-Personal" "https://posteo.de/calendars/ics/n2onw8jtaxxomy9x2p9p9thox7eitp7w" "Red"))))
(defun calendar-init ()
