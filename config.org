...
(package! closql
  ;;:recipe (:host github :repo "magit/closql" :files ("*.el"))
  :pin "0a7226331ff1f96142199915c0ac7940bac4afdd")
#+end_src
...
#+begin_src emacs-lisp :tangle no
(defvar required-fonts '("JetBrainsMono Nerd Font.*" "Overpass" "JuliaMono" "IBM Plex Mono" "Merriweather" "Alegreya"))
...
(use-package! org-noter
  ...
  (map! :map org-noter-doc-mode-map
        ...
        "C-M-i" #'org-noter-insert-precise-note-toggle-no-questions))
#+end_src


*** Djvu (for org-noter)

#+call: confpkg("!Pkg djvu")
...
#+begin_src emacs-lisp
(set-company-backend! 'text-mode '(company-wordfreq))
#+end_src
...
(defun company-ispell-german ()
  (interactive)
  (setq-local company-ispell-dictionary (file-truename "~/.config/dictionaries/wordlist_german.txt")))
...
For now arch packages of hunspell (de, en_us) are required.
...
(package! flycheck-golangci-lint :recipe (:host github :repo "weijiangan/flycheck-golangci-lint")
  :pin "9def093e416e9a6ddd3cae8590dbb7ff6314925a")
#+end_src
...
(use-package! string-inflection
  ...
  :init
  (map! :leader :prefix ("cn" . "naming convention")
        :desc "next" "n" #'string-inflection-all-cycle
...
(setq! variable-pitch-serif-font (font-spec :family "Source Serif 4" :size 27))
...
# This is the original version used by TEC.
# #+begin_src emacs-lisp :tangle packages.el
...
(setq org-directory "~/org" ; Let's put files here.
      ...
      ;;org-agenda-files (list "~/org/gtd")                  ; Seems like the obvious place.
...
#+begin_src emacs-lisp
(after! oc-csl
  (setq org-cite-csl-styles-dir "~/Zotero/styles/iso690"))
#+end_src
...
(setq org-todo-keywords
      '((sequence "NEXT(n)" "TODO(t)" "PROJ(p)" "LOOP(r)" "STRT(s)" "WAIT(w)" "HOLD(h)" "IDEA(i)" "|" "DONE(d)" "KILL(k)")
        (sequence "[ ](T)" "[-](S)" "[?](W)" "|" "[X](D)")
        (sequence "|" "OKAY(o)" "YES(y)" "NO(n)"))
      ;;org-todo-keywords-for-agenda '((sequence "CNCL" "DONE" "WAIT" "TODO" "NEXT"))
...
(setq org-agenda-custom-commands
      '(
        ...
        ("d" "Daily review" agenda ""
         ((org-agenda-span 'day)
          (org-agenda-start-day nil)
          (org-agenda-start-on-weekday nil)
          (org-agenda-start-with-log-mode 'only)
          (org-agenda-log-mode-items '(state clock))))
        ("n" "Scheduled today and all NEXT items...."
         ((agenda "" ((org-agenda-span 3)
                      (org-agenda-start-day nil)))
          (todo "LOOP" ((org-agenda-overriding-header "Take a break")))
...
(plist-put org-format-latex-options :zoom 1) ; Calibrated based on the TeX font and org-buffer font.
#+end_src
...
#+begin_src emacs-lisp
(defvar org-latex-default-fontset 'cm
...
#  (setq type-break-interval (* org-pomodoro-length 60) ;; 25 mins
