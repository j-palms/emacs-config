...
#+begin_src emacs-lisp
(setq
      doom-font (font-spec :family "Iosevka Nerd Font" :size 24)
      ...
      doom-variable-pitch-font (font-spec :family "IBM Plex Sans" :size 26)
      doom-unicode-font (font-spec :family "JuliaMono Nerd Font")
      doom-emoji-font (font-spec :family "Noto Emoji") ; Just used by me
...
#+begin_src emacs-lisp :noweb-ref none
(setq required-fonts '("Iosevka Nerd Font" "Overpass" "JuliaMono Nerd Font" "IBM Plex Serif" "IBM Plex Sans"
                       "Merriweather" "Alegreya" "Noto Emoji"))
...
[[file:misc/splash-images/monkey_small.svg]]
...
(defvar fancy-splash-image-template
  (expand-file-name "monkey_small.svg" fancy-splash-image-directory)
...
(defvar fancy-splash-template-colours
  '(("#111112" :face default   :attr :foreground)
    ...
    ("#000000" :face modus-themes-heading-2))
...
# CUSTOM: THIS IS REPLACED WITH OFFICIAL VERSION
# #+begin_src systemd
# [Unit]
...
;; (setq! variable-pitch-serif-font (font-spec :family "Merriweather" :size 27))
;; (setq! variable-pitch-serif-font (font-spec :family "Source Serif Pro" :size 27))
(setq! variable-pitch-serif-font (font-spec :family "IBM Plex Serif" :size 27))
...
Here im using TEC's  ~org-latex-preview-auto-mode~ development branch.
But since recently this breaks ~org-capture~. So I deactivate it for now..
...
(use-package! org-modern
  ...
  (setq org-modern-star '("◉" "○" "✸" "✿" "✤" "✜" "◆" "▶")
        ...
        '(("TODO" :inverse-video nil :inherit org-todo)
          ("NEXT" :inverse-video nil :inherit +org-todo-active)
          ("HOLD" :inverse-video nil :inherit +org-todo-onhold)
          ("WAIT" :inverse-video nil :inherit +org-todo-onhold)
          ("CNCL" :inverse-video nil :inherit +org-todo-cancel)
          ("DONE" :inverse-video nil :inherit +org-todo-done))
...
CUSTOM:  revert to default behaviour:
- ~visual-line-mode~ =on=
- ~auto-fill-mode~ =off=
...
(pdf-tools-install)  ;; always use pdf-tools
...
;; (setq lsp-modeline-code-action-fallback-icon 'display (nerd-icons-mdicon "nf-md-lightbulb_on_10" :face 'nerd-icons-dyellow))
...
(map! ("C-c e"
       :desc "Go to next error" #'flymake-goto-next-error))
(map! ("C-c n"
       :desc "Go to prev error" #'flymake-goto-prev-error))
...
(map! ("C-<up>"
       :desc "Go to previous visible line" #'evil-previous-visual-line)
      ("C-<down>"
       :desc "Go to next visible line" #'evil-next-visual-line))
...
;; Set word wrapping hooks for text and programming modes.
(remove-hook 'prog-mode-hook #'visual-line-mode)
(add-hook 'prog-mode-hook #'auto-fill-mode)

;; Set max line length for go
...
I'm using different environments for work & private use.
# The variable ~work-environment~ is considered at the appropriate places for adjusting the settings for the environment that is currently in use.
I changed the behaviour. Now I have to put the relevant settings right here.

#+begin_src emacs-lisp
(defun custom/work-environment ()
  (interactive)
  (setq org-gtd-directory "~/org/gtd/work")
  (setq org-roam-filter-by-entries '("private"))
  (setq work-environment t))

(defun custom/private-environment ()
  (interactive)
  (setq org-gtd-directory "~/org/gtd/private")
  (setq org-roam-filter-by-entries '("work"))
  (setq work-environment nil))

(defun custom/toggle-environment ()
  (interactive)
  (if work-environment
...
(map! :leader
      (:prefix "e"
       :desc "Toggle private <-> work environment" "t"  #'custom/toggle-environment
       :desc "Set private environment"             "p"  #'custom/private-environment
       :desc "Set work environment"                "w"  #'custom/work-environment))
...
(use-package! org-roam
  ...
  (map! :leader
        ...
        :desc "org-roam-node-find filtered" "f" #'custom/org-roam-node-find-actions
...
** org-roam-filter-entries

#+call: confpkg("!Pkg org-roam-filter-entries")
...
;;; 0. Variables
;;  --------------------------------------------------
(defcustom org-roam-filter-by-entries '("private")
...
"
  ...
  (let ((numeric-arg (prefix-numeric-value arg)))
    (cond
     ((null arg) (custom/org-roam-node-find))
     ((= numeric-arg 1) (custom/org-roam-node-find-only))
     ((= numeric-arg 2) (custom/org-roam-node-find-entry))
     ((= numeric-arg 3) (org-roam-node-find))
...
(use-package! org-pomodoro
  :after org
  :hook ((org-pomodoro-started . custom/load-window-config-and-close-work-agenda)
         (org-pomodoro-finished . custom/save-window-config-and-show-work-agenda))
  :config
  (custom/pomodoro-long)
  (map! :prefix ("C-c SPC" . "org-pomodoro")
        :desc "Start"        "SPC"  #'org-pomodoro
        :desc "Set to long"         "l"  #'custom/pomodoro-long
        :desc "Set to mid"         "m"  #'custom/pomodoro-mid
        :desc "Set to short"         "s"  #'custom/pomodoro-short)
  (setq org-pomodoro-manual-break t)
  :init
  (defun custom/pomodoro-long ()
    (interactive)
    (setq org-pomodoro-length 90
          org-pomodoro-short-break-length 30
          org-pomodoro-long-break-length 45))

  (defun custom/pomodoro-mid ()
    (interactive)
    (setq org-pomodoro-length 60
          org-pomodoro-short-break-length 15
          org-pomodoro-long-break-length 30))

  (defun custom/pomodoro-short ()
    (interactive)
    (setq org-pomodoro-length 25
          org-pomodoro-short-break-length 5
          org-pomodoro-long-break-length 15))

  (defun custom/save-window-config-and-show-work-agenda ()
    (interactive)
  ...
  (defun custom/load-window-config-and-close-work-agenda ()
    (interactive)
    (org-save-all-org-buffers)
    (jump-to-register ?`)))
#+end_src
...
Some tries for environment changes..
# #+begin_src emacs-lisp :tangle yes
# ;; (defun set-org-gtd-environment (is-work-environment)
#  ;;   (if is-work-environment
#  ;;       (setq org-gtd-directory "~/org/gtd/work")
#  ;;     (setq org-gtd-directory "~/org/gtd/private")))
#
#  ;; (set-org-gtd-environment work-environment)
#  ;; (add-variable-watcher work-environment (lambda (symbol newval op where) ((set-org-gtd-environment newval))))
# #+end_src
  ...
  (lsp-register-client
   (make-lsp-client :new-connection
		    (lsp-stdio-connection
                    ...
                    :initialization-options (lambda () `((sphinx . ((confDir . "/home/jpalms/code/ark/doc")
                                                                    (buildDir . "/home/jpalms/code/ark/doc/build")
                                                                    (srcDir . "/home/jpalms/code/ark/doc")))
...
#+begin_src emacs-lisp
(setq! cape-dict-file "~/.config/doom/misc/dict/en")

(defun custom/toggle-cape-dict-en-ger ()
    (interactive)
    (if (equal cape-dict-file "~/.config/doom/misc/dict/en")
        (setq! cape-dict-file "~/.config/doom/misc/dict/de")
      (setq! cape-dict-file "~/.config/doom/misc/dict/en")))

(map! "C-c l" #'custom/toggle-cape-dict-en-ger)

;; (add-hook 'text-mode-hook (lambda () (add-hook 'completion-at-point-functions #'cape-dict nil t)))
#+end_src

** phscroll

#+call: confpkg("!Pkg phscroll")
